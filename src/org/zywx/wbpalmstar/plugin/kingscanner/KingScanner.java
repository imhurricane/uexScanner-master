package org.zywx.wbpalmstar.plugin.kingscanner;
...
import org.zywx.wbpalmstar.plugin.kingzxing.DataJsonVO;
import org.zywx.wbpalmstar.plugin.kingzxing.ScannerUtils;
...
public class KingScanner extends EUExBase {
    ...
    public KingScanner(Context context, EBrowserView view) {
        super(context, view);
...
//        if (resultCode == Activity.RESULT_OK) {
            try {
                ...
                jobj.put(EUExCallback.F_JK_CODE,
                        data.getStringExtra(Intents.Scan.RESULT).replace("\"", "\\\""));
                jobj.put(EUExCallback.F_JK_TYPE, "QR_CODE");
                String result = jobj.toString();
//                Toast.makeText(mContext,result,Toast.LENGTH_LONG).show();
                if (null != openFuncId) {
                    callbackToJs(Integer.parseInt(openFuncId), false, EUExCallback.F_C_SUCCESS, result);
                } else {
